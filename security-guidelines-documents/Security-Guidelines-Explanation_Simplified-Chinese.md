...
**进阶攻击手法：为什么 A 输入的代码能窃取 B 的资料？**

很多人会困惑："攻击者并没有修改我的网站，为什么能偷到其他用户的资料？"让我用一个完整的例子来解释：
...
**简单比喻**
想象你的网站是一家银行：
- 攻击者 A 在银行大厅放了一张"假取款单"（恶意脚本）
- 客户 B 以为是真的，填写了密码
- A 就得到了 B 的密码

XSS 就是让攻击者能在你的"银行大厅"（网站）放置"假取款单"（恶意代码）。

这就是为什么要用 `htmlspecialchars()` - 它能确保所有用户输入都被当作纯文本显示，而不是可执行的代码。
